...
from models.loss_func import loss_func_dict
...
from utils.traj_utils import (
    ...
    traj_local2global_heading, get_init_heading_q, convert_traj_world2heading, convert_traj_heading2world,
    convert_traj_world2swing,
    traj_local2global_swing,
    convert_twist_swing,
    convert_traj_world2swing_orig,
    convert_twist_swing_orig,
    convert_traj_world2st_orig,
    convert_traj_world2st_single_trans,
    traj_local2global_st
)
...
class DataEncoder(nn.Module):
    ...
    def __init__(self, cfg, specs, ctx, **kwargs):
        ...
        if self.input == 'local_traj':
            cur_dim = 11
        elif self.input == 'global_swing_twist':
            # swing: 6d, translation: 3d, twist: 1d
    ...
    def forward(self, data):
        ...
        elif self.input == 'global_swing':
            swing, init_heading_trans = convert_traj_world2swing_orig(data['orient_q_tp'], data['trans_tp'])
            if self.orient_type == 'axis_angle':
                swing = quaternion_to_angle_axis(swing)
            elif self.orient_type == '6d':
                swing = quat_to_rot6d(swing)
            x_in = torch.cat([init_heading_trans, swing], dim=-1)
        elif self.input == 'global_swing_twist':
            swing, init_heading_trans, twist = convert_traj_world2st_single_trans(data['orient_q_tp'], data['trans_tp'])
            if self.orient_type == 'axis_angle':
                swing = quaternion_to_angle_axis(swing)
            elif self.orient_type == '6d':
                swing = quat_to_rot6d(swing)
            x_in = torch.cat([init_heading_trans, swing, twist], dim=-1)
        else:
...
class DataDecoder(nn.Module):
    ...
    def __init__(self, cfg, specs, ctx, **kwargs):
        ...
        self.angle_act = nn.Tanh()
        self.prob_act = nn.Sigmoid()
        ...
        """ multi head """
        if 'multi_head' in specs:
            multi_head_cfg = specs['multi_head']
            self.multi_head = nn.ModuleList()
            self.out_head = nn.ModuleList()
            for _ in range(multi_head_cfg['num_heads']):
                net = MLP(before_mlp_dim, multi_head_cfg['hdim'], ctx['mlp_htype'])
                self.multi_head.append(net)
                out_layer = nn.Linear(net.out_dim, 2) # predict angle and the uncertainty for now
                self.out_head.append(out_layer)
        else:
    ...
    def forward(self, data, mode, sample_num=1):
        ...
        data[f'{mode}_out_trans_tp'], data[f'{mode}_out_orient_q_tp'] = traj_local2global_heading(out_local_traj_tp, 
...
class TrajPredVAE(pl.LightningModule):
    ...
    def init_batch_data(self, batch):
        ...
        if 'trans' in data:
            ...
            # logger.debug(f"Initial local traj: {data['local_traj_tp'][0, ..., -2:]}")
        # in_joint_pos
    ...
    def training_step(self, batch, batch_idx):
        ...
        return {'loss': loss, 'log': loss_uw_dict}
