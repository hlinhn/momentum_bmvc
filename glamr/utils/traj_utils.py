...
def transform_to_upright(orient):
    upright = torch.zeros_like(orient).float()
    upright[..., :2] = torch.sqrt(torch.tensor(2.0).float()) / 2
    tf = quat_mul(quat_mul(quat_conjugate(upright), orient), upright)
    return tf


def global_rot_twist(grot):
    u = grot[..., -1]
    n = 1
    m = grot[..., 0] * n
    l = torch.sqrt(m ** 2 + u ** 2 * n)
    twist = torch.stack([m/l, torch.zeros_like(m/l), 
                        torch.zeros_like(m/l), u/l], axis=-1)
    swing = quat_mul(grot, quat_conjugate(twist))
    return twist, swing


def convert_twist_swing(orient):
    orient_zup = transform_to_upright(orient)
    twist, swing = global_rot_twist(orient_zup)
    return twist, swing


def convert_twist_swing_orig(orient):
    base_orient = [0.5, 0.5, 0.5, 0.5]
    base_orient = torch.tensor(base_orient, device=orient.device)
    orient_nobase = quat_mul(orient, quat_conjugate(base_orient).expand_as(orient))
    heading_q = get_heading_q(orient_nobase)
    swing = deheading_quat(orient_nobase, heading_q)
    twist = torch.stack([heading_q[..., -1], heading_q[..., 0]], dim=-1)
    return twist, swing


def get_orientation_from_twist_swing(twist, swing):
    base_orient = [0.5, 0.5, 0.5, 0.5]
    base_orient = torch.tensor(base_orient, device=swing.device)
    twist_quat = torch.zeros_like(swing)
    twist_quat[..., -1] = torch.cos(twist / 2)
    twist_quat[..., 0] = torch.sin(twist / 2)
    upright = quat_mul(twist_quat, swing)
    quat = quat_mul(upright, base_orient.expand_as(upright))
    return quat


def convert_traj_world2swing_orig(orient, trans):
    base_orient = [0.5, 0.5, 0.5, 0.5]
    base_orient = torch.tensor(base_orient, device=orient.device)
    orient_nobase = quat_mul(orient, quat_conjugate(base_orient).expand_as(orient))
    heading_q = get_heading_q(orient_nobase)
    swing = deheading_quat(orient_nobase, heading_q)
    inv_twist = quat_conjugate(heading_q).expand_as(orient_nobase)
    trans_local = trans.clone()
    trans_local[..., :2] -= trans[0, ..., :2]
    trans_heading = quat_apply(inv_twist, trans_local)
    return swing, trans_heading


def convert_traj_world2st_orig(orient, trans):
    base_orient = [0.5, 0.5, 0.5, 0.5]
    base_orient = torch.tensor(base_orient, device=orient.device)
    orient_nobase = quat_mul(orient, quat_conjugate(base_orient).expand_as(orient))
    heading_q = get_heading_q(orient_nobase)
    ...
    trans_local[..., :2] -= trans[0, ..., :2]
    trans_heading = quat_apply(inv_twist, trans_local)
    twist_angle = 2 * torch.atan2(heading_q[..., -1], heading_q[..., 0])
    twist_angle -= twist_angle[[0]]
    twist_sin = torch.sin(twist_angle)
    twist_cos = torch.cos(twist_angle)
    twist = torch.stack([twist_sin, twist_cos], dim=-1)
    return swing, trans_heading, twist


def convert_traj_world2st_single_trans(orient, trans):
    base_orient = [0.5, 0.5, 0.5, 0.5]
    base_orient = torch.tensor(base_orient, device=orient.device)
    orient_nobase = quat_mul(orient, quat_conjugate(base_orient).expand_as(orient))
    heading_q = get_heading_q(orient_nobase)
    swing = deheading_quat(orient_nobase, heading_q)
    inv_initial_twist = quat_conjugate(heading_q[[0]]).expand_as(orient_nobase)
    trans_local = trans.clone()
    trans_local[..., :2] -= trans[0, ..., :2]
    trans_heading = quat_apply(inv_initial_twist, trans_local)
    twist_angle = 2 * torch.atan2(heading_q[..., -1], heading_q[..., 0])
    twist_angle -= twist_angle[[0]]
    twist_sin = torch.sin(twist_angle)
    twist_cos = torch.cos(twist_angle)
    twist = torch.stack([twist_cos, twist_sin], dim=-1)
    return swing, trans_heading, twist


def convert_traj_world2swing(orient, trans):
    orient_zup = transform_to_upright(orient)
    twist, swing = global_rot_twist(orient_zup)
    inv_twist = quat_conjugate(twist).expand_as(orient_zup)
    trans_local = trans.clone()
    trans_local[..., :2] -= trans[0, ..., :2]
    trans_heading = quat_apply(inv_twist, trans_local)
    return swing, trans_heading
...
def traj_local2global_swing(local_traj, deheading_local=False, local_orient_type='6d', local_heading=True):
    d_xy_yawcoord, z = local_traj[..., :2], local_traj[..., 2]
    local_orient, d_heading_vec = local_traj[..., 3:-2], local_traj[..., -2:]
    d_heading = vec_to_heading(d_heading_vec)
    if local_heading:
...
def traj_local2global_st(local_traj, deheading_local=False, local_orient_type='6d', local_heading=True):
    d_xy_yawcoord, z = local_traj[..., :2], local_traj[..., 2]
    local_orient, d_heading_vec = local_traj[..., 3:-2], local_traj[..., -2:]
    d_heading = vec_to_heading(d_heading_vec)
    if local_heading:
        ...
        heading = d_heading
    d_xy = d_xy_yawcoord.clone()
    d_xy[1:] = rot_2d(d_xy_yawcoord[1:], heading[[0]])
    xy = torch.cumsum(d_xy, dim=0)
    trans = torch.cat([xy, z.unsqueeze(-1)], dim=-1)
    if local_orient_type == '6d':
        local_q = rot6d_to_quat(local_orient)
